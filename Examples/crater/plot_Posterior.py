...
def plotPosterior(fname, pos_likl, pos_rain, pos_erod):
		nb_bins=50
		...
		n, rainbins, patches = ax1.hist(pos_rain, bins=nb_bins, alpha=0.5, facecolor='sandybrown', normed=True)
		...
		n, erodbins, patches = ax1.hist(pos_erod, bins=nb_bins, alpha=0.5, facecolor='sandybrown', normed=True)
		...
		ax2.set_facecolor('#f2f2f3')
		ax2.plot(slen,pos_likl,linestyle='-', linewidth= width, color='k', label=None)
		ax2.set_title(r'Trace of Rain',size= font+2)
		ax2.set_xlabel('Samples',size= font+1)
		ax2.set_ylabel(r'$\varepsilon$', size= font+1)
		ax2.set_xlim([0,np.amax(slen)])
...
def main():
	...
	print len(likl)
	print likl_.shape
	plotPosterior(fname, likl_, rain_, erod_)
