...
def main():
	...
	input_file = np.loadtxt('data/etopo.txt')
	run_nb_str = 'mcmcresults_' + str(run_nb)

	etopo_mcmc = etopo_MCMC(simtime, samples, input_file, filename, xmlinput, erodlimts, rainlimits, mlimit, nlimit, run_nb_str)
	[pos_rain, pos_erod, pos_m, pos_n, pos_tau, pos_rmse, accept_ratio, accepted_count] = etopo_mcmc.sampler()
