...
def plotFunctions(fname, pos_likl, pos_rain, pos_erod, pos_taus):
	...
	hist, rainedges, erodedges = np.histogram2d(pos_rain, pos_erod)#, normed =True , bins = 40 , range = [[rainmin, rainmax],[erodmin, erodmax]])
	...
	trace1 = go.Scatter3d(x=rainpos, y=erodpos, z=dz, mode='markers', marker=dict(size=12, color = dz, colorscale='Cividis', showscale=True))
	data = [trace1]
	layout = go.Layout(title='Joint distribution rain, erod',autosize=True, scene=Scene(
