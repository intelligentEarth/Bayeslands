...
def topoGenerator(inputname, rain, erodibility, m, n, etime):
    ...
    regz = elevArray(XYZ)
    mat=np.matrix(regz)
    np.savetxt('data/generated.txt',mat,fmt='%.5f')

    return

def inputVisualisation(inputname, rain, erodibility, m, n, etime = 0):
    model = badlandsModel()
    # Define the XmL input file
    ...
    elev = np.loadtxt('data/generated.txt')
...
def main():
    ...
    choice = input('Would you like to: \n 1) Generate a final topography \n 2) Visualise the initial topography \n ')

    if choice == 1:
        topoGenerator('crater.xml', 2, 5.e-4, 0.5, 1, 5000)
    else:
        topoGenerator('crater.xml', 2, 5.e-4, 0.5, 1, 0)
        inputVisualisation('crater.xml', 2, 5.e-4, 0.5, 1, 0)
