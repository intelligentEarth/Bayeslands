...
class Crater_MCMC():
	...
	def sampler(self):
		...
		eta_erodep = np.log(np.var(init_pred_elev_vec[self.simtime] - real_elev))

		self.step_eta_elev = eta_elev*0.05
		self.step_eta_erodep = eta_erodep*0.05

		tau_elev_pro = np.exp(eta_elev)
		tau_erodep_pro = np.exp(eta_erodep)
		...
		[likelihood, pred_elev, pred_erodep] = self.likelihood_func(v_proposal, real_elev, real_erodep, tau_elev_pro, tau_erodep_pro)
		...
		pos_tau_elev = np.full(samples, tau_elev_pro)
		pos_tau_erodep = np.full(samples,tau_erodep_pro)
		...
		for i in range(samples-1):
			...
			print 'eta_elev_pro', eta_elev_pro, 'eta_erodep_pro', eta_erodep_pro
			print 'tau_elev_pro ', tau_elev_pro, 'tau_erodep_pro', tau_erodep_pro
			...
			if u < mh_prob: # Accept sample
				...
				print 'Sample ', i, ' rejected and reta_elevined'
...
def main():
	...
	[pos_rain, pos_erod, pos_m, pos_n, pos_tau_elev, pos_tau_erodep , pos_likl, accept_ratio, accepted_count] = crater_mcmc.sampler() #pos_rmse,
