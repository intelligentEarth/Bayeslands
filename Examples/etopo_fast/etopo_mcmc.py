...
class etopo_MCMC():
	def __init__(self, muted, simtime, samples, real_elev , real_erodep, filename, xmlinput, erodlimits, rainlimits, mlimit, nlimit, run_nb):
		...
		self.sim_interval = np.arange(0, self.simtime+1, 50000)
	...
	def sampler(self):
		...
		self.viewGrid('real', 0 , 1.5, 5.e-6, width=1000, height=1000, zData=real_elev, title='Real Elevation')
		self.viewMap('real', 0 , 1.5, 5.e-6, width=1000, height=1000, zData=real_erodep, title='Real Erodep')
...
def main():
	...
	[pos_rain, pos_erod, pos_m, pos_n, pos_tau_elev, pos_tau_erodep , pos_likl, accept_ratio, accepted_count, count_list] = etopo_mcmc.sampler() #pos_rmse,
